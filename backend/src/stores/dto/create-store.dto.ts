...
export class CreateStoreDto {
  ...
  @IsNotEmpty()
  @Length(2, 50)
  @Matches(/^[A-Z0-9_]+$/, { message: '门店编码只能包含大写字母、数字和下划线' })
  code: string;
  ...
  @IsString()
  @IsNotEmpty()
  @Matches(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, { message: '开门时间格式应为 HH:mm' })
  openTime: string;

  @ApiProperty({ description: '关门时间', example: '23:00' })
  @IsString()
  @IsNotEmpty()
  @Matches(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, { message: '关门时间格式应为 HH:mm' })
  closeTime: string;

  @ApiProperty({ description: '门店描述', example: '位于CBD核心区域的旗舰店', required: false })
  @IsOptional()
  @IsString()
  @Length(0, 500)
  description?: string;

  @ApiProperty({ description: '门店照片URL列表', example: ['https://example.com/store1.jpg'], required: false })
  @IsOptional()
  images?: string[];

  @ApiProperty({ description: '设施列表', example: ['器械区', '有氧区', '团课室'], required: false })
  @IsOptional()
  facilities?: string[];
